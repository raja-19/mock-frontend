...
function processValueChange(el) {
    ...
    alert(pending["achternaam"].nl);
    pending[field].nl = nl.value;
    pending[field].en = en.value;
    pending[field].message = message.value;
    ...
    requestChangesButton.disabled = nl.value == current[field].nl && en.value == current[field].en;
} 
...
function discardChanges(el) {
    ...
    pending[field].message = "";
    pending[field].nl = current[field].nl;
    pending[field].en = current[field].en;
    document.cookie = `pending=${JSON.stringify(pending)}`;
    var message = document.getElementById("message");
    message.value = "";

    nl.value = current[field].nl;
    en.value = current[field].en;
...
function requestChanges(el) {
    ...
    current[field].nl = pending[field].nl;
    current[field].en = pending[field].en;
...
function devtoolsSelect(el) {
    ...
    var field = el.id;
    var fieldBody = templateBody.replaceAll("$Field", field.charAt(0).toUppercase + field.slice(1)).replace("$field", field);
    ...
    nl.value = pending[field].nl; 
    en.value = pending[field].en; 
    message.value = pending.message;

    requestChangesButton.disabled = pending[field].nl == current[field].nl && pending[field].en == current[field].en;
}
